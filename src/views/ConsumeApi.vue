<template>
  <div id="consumeapi">
    ...
    <v-card class="container text-center espacio" color="orange darken-2" dark>
    ...
    <v-container fluid>
      <v-row justify="center">
        ...
        <v-card class="text-center">
          ...
          <v-btn color="primary" @click="show = !show, say(), hello()" v-if="!show">Calcular</v-btn>
          ...
          <transition class="slide-fade-leave-active">
            ...
            <p v-else>{{ data }} <br> {{ dataPost }} </p>
          </transition>
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="primary" @click="say(), hello()">Otra vez</v-btn>
          <v-btn color="primary" @click="dialog = false, limpiar(), show = !show">Cerrar</v-btn>
          <v-spacer></v-spacer>
        </v-card-actions>
        </v-card>
      </v-dialog>
...
<script>
 ...
 import OtroComponente from './Home';
    ...
    export default {
        ...
        methods: {
            say () {
            axios({ method: "GET", "url": "http://localhost:3000/index" }).then(result => {
            ...
            hello () {
            axios({ method: "POST", "url": "http://localhost:3000/theend" }).then(result => {
